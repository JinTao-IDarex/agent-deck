...
>(({ side = 'right', className, children, ...props }, ref) => (
  <SheetPortal>
    ...
    <DialogPrimitive.Content
      ref={ref}
      className={cn(
        'fixed z-50 flex h-full w-full flex-col border-border bg-card shadow-placard transition ease-in-out data-[state=closed]:duration-200 data-[state=open]:duration-300 sm:max-w-xl',
        side === 'right' &&
          'inset-y-0 right-0 border-l data-[state=closed]:slide-out-to-right data-[state=open]:slide-in-from-right rounded-l-2xl',
        side === 'left' &&
...
const SheetHeader = ({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) => (
  <div className={cn('flex flex-col gap-1.5 border-b border-border p-4 pr-12', className)} {...props} />
);
