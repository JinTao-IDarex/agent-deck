...
function resolveProjectRoot(req) {
    ...
    const fromBody = req.body && typeof req.body.projectRoot === 'string' && req.body.projectRoot.trim();
...
app.get('/api/artifacts', (req, res) => {
    ...
    let list = artifacts;
    if (req.query.agent)
        list = list.filter((a) => a.agent === req.query.agent);
    if (req.query.scope)
        list = list.filter((a) => a.scope === req.query.scope);
    if (req.query.type)
        list = list.filter((a) => a.type === req.query.type);
...
app.post('/api/validate', (req, res) => {
    ...
    const parsed = parseArtifactContent(content, type, nameHint);
    res.json(validateArtifact({
        agent,
        type: type,
        name: parsed.name,
...
app.get('/api/outdir', (req, res) => {
    const projectRoot = resolveProjectRoot(req);
    const agent = String(req.query.agent || 'claude');
    const type = String(req.query.type || 'skill');
    const scope = String(req.query.scope || 'project');
    if (!AGENT_KINDS.includes(agent)) {
        return res.status(400).json({ error: 'invalid agent' });
    }
    res.json({ outDir: defaultOutDir(agent, type, scope, projectRoot) });
});
function serializeArtifact(a, { includeBody = false } = {}) {
    const base = {
        id: a.id,
        type: a.type,
        agent: a.agent,
