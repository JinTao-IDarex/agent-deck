...
export const AGENT_KINDS = Object.keys(AGENT_META);
...
export function defaultOutDir(agent, type, scope, projectRoot) {
    if (type === 'skill') {
        switch (agent) {
            case 'claude':
            case 'mimo':
...
export function getSpec(agent) {
    ...
    return {
        ...
        skill: {
            ...
            optionalFrontmatter: agent === 'mimo' || agent === 'claude'
                ? ['icon']
