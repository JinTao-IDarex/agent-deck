...
export function validateArtifact(input) {
    const issues = [];
    const meta = AGENT_META[input.agent];
    if (!meta) {
        return {
            agent: input.agent,
            type: input.type,
...
export function validateScannedArtifact(artifact) {
    const hasLocales = artifact.type === 'skill' &&
        fs.existsSync(path.join(artifact.dir, 'locales', 'zh-CN.json')) &&
        fs.existsSync(path.join(artifact.dir, 'locales', 'en-US.json'));
    const hasOpenaiYaml = artifact.type === 'skill' && fs.existsSync(path.join(artifact.dir, 'agents', 'openai.yaml'));
    return {
