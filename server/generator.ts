...
import type { Artifact, GenerateInput, GenerateResult } from './types.js';

function dumpFrontmatter(obj: Record<string, unknown>): string {
  return yaml.dump(obj, { lineWidth: 100, noRefs: true }).trim();
}

export function generateArtifact(input: GenerateInput): GenerateResult {
  ...
  let mainFm: Record<string, unknown> = {};
  ...
  if (type === 'skill') {
    ...
    if (agent === 'qoder') {
      ...
      fm.tools = tools;
      if (skills.length) fm.skills = skills;
...
export function artifactToGenerateInput(artifact: Artifact, targetAgent: string): GenerateInput {
  return {
    agent: targetAgent as GenerateInput['agent'],
    type: artifact.type,
